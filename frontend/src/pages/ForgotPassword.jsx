...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (data.sandbox_reset_token) {
           setMessage(`Sandbox mode token: ${data.sandbox_reset_token}`);
        }
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-cyber-bg flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-cyber-warning/5 rounded-full blur-[100px] pointer-events-none" />
      ...
      <div className="max-w-md w-full glass p-8 rounded-3xl shadow-[0_0_50px_rgba(99,102,241,0.1)] relative z-10 border border-cyber-border/80">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          {message && (
            <p className="text-xs text-cyber-success text-center mt-2 font-mono break-words">{message}</p>
          )}
          {error && (
        ...
        <div className="mt-6 flex justify-center">
          <Link to="/login" className="flex items-center gap-1 text-[11px] text-zinc-400 hover:text-white transition-colors">
